...
// GET: List all products for partner's shop
...
// POST: Create a product for partner's shop
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validate fields
    const name = requireString(prodFields.name, '商品名').slice(0, 200)
    const description = sanitizeString(prodFields.description, 5000) || null
    const price = validatePositiveInt(prodFields.price, '価格')
    const stock = validatePositiveInt(prodFields.stock ?? 0, '在庫数')
    const tax_rate = clampNumber(prodFields.tax_rate, 0, 1, 0.1)
    const images = Array.isArray(prodFields.images) ? prodFields.images.slice(0, 10) : []

    const admin = createAdminClient()

    // Create the product - always force shop_id to partner's shop
