...
import { Package, ShoppingCart, Users, TrendingUp, AlertTriangle, Handshake } from 'lucide-react'
...
export default async function AdminDashboard() {
  ...
  const monthCommissionTotal = monthCommissions?.reduce((sum, c) => sum + c.commission_amount, 0) ?? 0
  ...
  const stats = [
    ...
    { label: '顧客数', value: `${totalCustomers ?? 0}人`, icon: Users, color: 'text-orange-600 bg-orange-50' },
    { label: '今月のコミッション', value: formatPrice(monthCommissionTotal), icon: Handshake, color: 'text-teal-600 bg-teal-50' },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200">
          ...
          {lowStockProducts && lowStockProducts.length > 0 ? (
            ...
            <div className="p-8 text-center text-sm text-gray-400">在庫少の商品はありません</div>
          )}
        </div>
      </div>
    </div>
